...
contract VehicleNFT is ERC721URIStorage, Ownable {
    ...
    // Change: The constructor now passes msg.sender as the initial owner for Ownable
    ...
    // Change: Function added to allow the current vehicle owner to assign an authorized car service provider
    function assignCarServiceProvider(uint256 tokenId, address provider) external onlyCurrentOwner(tokenId) {
        require(authorizedCarServiceProviders[provider], "Not an authorized car service provider");
        vehicles[tokenId].ownerAddress = provider;
        emit CarServiceProviderAuthorized(provider);
    }
    ...
    // Function `getVehicle` was removed, as the `vehicles` mapping is public and can be accessed directly
}
